...
// --- CORRECT: Import the new, reliable recorder hook ---
...
import { MessageSquareText, Send, Paperclip, Video, PhoneOff, Loader2, FileText, Mic, StopCircle, PhoneIncoming } from 'lucide-react';
...
export default function UserMessage() {
    ...
    // --- CORRECT: Use the new hook for recording ---
    const {
    ...
    // --- CORRECT: This effect handles sending the voice note AFTER recording is done ---
    useEffect(() => {
    ...
    // --- All other functions (handleSendMessage, handleFileUpload, startCall, etc.) are correct ---
    const handleSendMessage = () => { /* ... no change ... */ };
    const handleFileUpload = async (event) => { /* ... no change ... */ };
    const startCall = () => { /* ... no change ... */ };
    const answerCall = () => { /* ... no change ... */ };
    const endCall = (shouldEmit = true) => { /* ... no change ... */ };
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-100 relative">
            {/* All overlay and modal UI for calls (no change) */}
            ...
            {stream && ( <div className="absolute top-0 left-0 w-full h-full bg-black bg-opacity-90 z-40 flex flex-col items-center justify-center p-4 gap-4"><div className="flex gap-4 w-full h-2/3">{callAccepted && <video playsInline ref={otherUserVideo} autoPlay className="w-full h-full object-cover rounded-lg" />}</div><video playsInline muted ref={myVideo} autoPlay className="absolute bottom-5 right-5 w-48 h-auto rounded-lg shadow-2xl border-2 border-white" /><button onClick={() => endCall(true)} className="absolute bottom-5 left-1/2 -translate-x-1/2 p-4 bg-red-600 text-white rounded-full"><PhoneOff /></button></div> )}
            ...
            {/* Main Chat Area (no change) */}
            <main className="flex-1 p-4 overflow-y-auto">
                <div className="flex flex-col gap-4">{messages.map((msg) => ( <div key={msg._id} className={`flex ${msg.sender._id === user.id ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[70%] p-3 rounded-lg shadow ${msg.sender._id === user.id ? 'bg-green-600 text-white' : 'bg-white text-black'}`}>{msg.sender._id !== user.id && (<div className="font-bold text-sm mb-1 text-green-700">{msg.sender.name || 'Admin'}</div>)}<MessageContent message={msg} /><div className="text-xs text-right mt-1 opacity-70">{new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div></div></div> ))}<div ref={messagesEndRef} /></div>
            </main>

            {/* Footer with CORRECTED recording buttons */}
            <footer className="p-4 bg-white border-t">
                <div className="flex items-center gap-3">
                    ...
                    {/* --- CORRECT: Use the new hook's status and functions --- */}
                    {recordingStatus === "recording" ? (
                        <button onClick={stopRecording} className="p-3 bg-red-500 text-white rounded-full animate-pulse"><StopCircle /></button>
                    ) : (
                        <button onClick={startRecording} className="p-3 bg-blue-500 text-white rounded-full"><Mic /></button>
                    )}
...
// NOTE: I've collapsed the unchanged functions for brevity. Make sure their implementations are correct.
// handleSendMessage, handleFileUpload, startCall, answerCall, endCall